...
with purchase_stats as (
  select
    ...
    sum(quantity) filter (where c.type = 'pearl') as total_pearls,
    sum(quantity) filter (where c.type = 'booster') as total_boosters,
...
-- Also account for NFTs received via non-purchase transfers (secondary market)
holdings as (
  select
  ...
  from (
    select to_address as addr, contract_id, sum(quantity) as net_qty from nft_transfers group by to_address, contract_id
    union all
    select from_address as addr, contract_id, -sum(quantity) as net_qty from nft_transfers group by from_address, contract_id
  ) balances
...
select
  ...
  coalesce(ps.total_spent_usd, 0) as total_spent_usd,
  coalesce(ps.total_spent_excluding_compounded_usd, 0) as total_spent_excluding_compounded_usd,
  coalesce(pay.total_earned_usd, 0) as total_earned_usd,
  12 + least(h.held_boosters * 0.5, 8) as effective_apr,
  coalesce(pay.total_earned_usd, 0) - coalesce(ps.total_spent_excluding_compounded_usd, 0) as net_position_usd
from holdings h
